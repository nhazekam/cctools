...
parser.add_option('--makeflow_gatk_loc',dest='gatkmf',type="string")
...
parser.add_option('--merge_jar',dest='merge_jar',type='string')
...
makeflow='haplotype_caller.makeflow'
...
if options.gatkmf:
	os.symlink(options.gatkmf, cur_dir+"/gatk_makeflow")
...
shutil.copyfile(options.merge_jar, cur_dir+"/MergeVcfs.jar")
shutil.copyfile(options.dict_jar, cur_dir+"/CreateSequenceDictionary.jar")
shutil.copyfile(options.java_zip, cur_dir+"/jre.zip")

os.chdir(cur_dir)

os.system('python gatk_makeflow --verbose -T ' +options.type+ ' ' + inputs + '--progeny_list progeny_file --makeflow ' +  makeflow + ' --out ' + output_vcf +' ' +' '.join(args))
	

os.system('makeflow ' + makeflow + ' -T wq -N ' + wq_project_name + ' -p 9123 -l ' + makeflow_log + ' -L ' + wq_log + ' -d all -o ' + debug_log+" --password mypwfile ")
